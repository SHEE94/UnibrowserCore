...
require(['jquery'], function($) {
	...
	$(".search-input").on("input propertychange", function() {
		...
		if (!wd) {
			...
			$(".empty-input").show();
			$(".search-btn").html(
				/^\b(((https?|ftp):\/\/)?[-a-z0-9]+(\.[-a-z0-9]+)*\.(?:com|net|org|int|edu|gov|mil|arpa|asia|biz|info|name|pro|coop|aero|museum|[a-z][a-z]|((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]\d)|\d))\b(\/[-a-z0-9_:\@&?=+,.!\/~%\$]*)?)$/i
				.test(wd) ? "进入" : "搜索");
	...
	function choice() {
		...
		$(".page-choice").on("transitionend", function(evt) {
			...
			$.ajax({
				...
				dataType: "json",
				success: function(res) {
					var url = 'https://www.bing.com' + res.images[0].url.replace(
						'1920x1080', '1080x1920');
